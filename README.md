...
Scenario workflows for running **[Kamiwaza](https://kamiwaza.ai)** on Kubernetes with the Kamiwaza Platform Operator. Each use case has explicit prerequisites, ordered steps, manifests, and verification.
...
| Namespace `kamiwaza` | Created by the cluster administrator. The operator never creates target namespaces.                          |
...
| Administrator-approved outbound TLS trust              | [security/tls-trust](security/tls-trust)                                           | #security #tls #pki #ca-trust #operator                     |
...
- **Merge patches** against the platform resource, and loadable administrator-policy fragments, rather than chart forks.
- **Lab defaults** clearly labeled: demo passwords, `*.kamiwaza.test`, `dc=kamiwaza,dc=local`.
