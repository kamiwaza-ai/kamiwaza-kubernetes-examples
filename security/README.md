...
| Policy integration  | Loadable administrator-policy fragments plus platform merge patches        |

## Scenario matrix

| Scenario                         | Identity source              | Authorization                     | TLS / mTLS                                                     | Key toggles / knobs                                                                | Main artifacts                                                                                                                  |
| -------------------------------- | ---------------------------- | --------------------------------- | -------------------------------------------------------------- | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| [cac](cac)                       | CAC/PIV certificate login    | ReBAC pre-admission               | client certificates at an external edge                        | auth profile `X509CAC` method                                                      | Auth profile fragment + platform patch + Gateway reference + evidence shapes                                                    |
| [rebac](rebac)                   | Keycloak JWT claims          | ReBAC, producer-owned grant edges | TLS                                                            | `spec.auth.rebac.*` on the platform resource                                       | Plan/diff/apply workflow script + `GrantManifest` examples (one refused on purpose) + platform patch + tenant registry fragment |
...
| [tls-trust](tls-trust)           | N/A (transport trust)        | N/A                               | Additive outbound TLS trust                                    | `transport.trust.additionalAuthorities[]`, `publicRoots`, `maximumBundleBytes`     | Loadable transport-policy fragment + exact-name Secret generator + refused inline-authority example                             |
...
| [rotation-drain](rotation-drain) | N/A                          | N/A                               | authority rotation, connection drain                           | `expirationPolicy`, `maxConnectionAge`, `transport.kamiwaza.ai/rotate-authority`   | Transport policy fragment + rotation request patch + one refused fragment                                                       |
