...
check_env_present "backend" "$BACKEND_ENV" "KAMIWAZA_BASE_URL"
...
# Turning verification ON only helps if the Kaizen backend's OWN
# KAMIWAZA_BASE_URL is reachable under verification. Some platform builds hand
# extensions a KAMIWAZA_API_URL/publicApiUrl that must be mirrored into
# KAMIWAZA_BASE_URL for the Python backend, and an internal HTTPS hostname still
# fails if it does not match the Traefik cert SANs. Fail closed on it.
info "3a. Backend reaches its own KAMIWAZA_BASE_URL with verification ON"
API_PROBE="$(kubectl -n "$EXT_NS" exec "$BACKEND_POD" -- python3 -c '
import httpx, os, sys
url = os.environ.get("KAMIWAZA_BASE_URL", "") or os.environ.get("KAMIWAZA_API_URL", "")
if not url:
    print("FAIL unset"); sys.exit(0)
...
case "$API_PROBE" in
  OK*)   pass "backend KAMIWAZA_BASE_URL verifies under verify-ON (HTTP ${API_PROBE#OK })" ;;
  SKIP*) pass "backend KAMIWAZA_BASE_URL is plain HTTP (TLS not applicable): ${API_PROBE#SKIP }" ;;
  *)     fail "backend cannot reach its KAMIWAZA_BASE_URL with verification ON (${API_PROBE#FAIL }). Kaizen's backend consumes KAMIWAZA_BASE_URL, not KAMIWAZA_API_URL directly. Set KAMIWAZA_BASE_URL to a cert-matching public origin (https://kamiwaza.test/api) or a safe internal HTTP API URL — see README." ;;
esac
