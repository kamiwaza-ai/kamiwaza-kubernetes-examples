...
while [ $# -gt 0 ]; do
  case "$1" in
  --ca-file)
    CA_FILES+=("${2:?--ca-file needs a path}")
    shift 2
    ;;
  --baseline-file)
    BASELINE_FILE="${2:?--baseline-file needs a path}"
    shift 2
    ;;
  --baseline-pod)
    BASELINE_POD="${2:?--baseline-pod needs a ref}"
    shift 2
    ;;
  --baseline-container)
    BASELINE_CONTAINER="${2:?--baseline-container needs a name}"
    shift 2
    ;;
  --no-baseline)
    NO_BASELINE=1
  ...
  --no-root-ca)
    ...
    ;;
  --namespace)
    NAMESPACES+=("${2:?--namespace needs a name}")
    shift 2
    ;;
  --include-sandboxes)
    INCLUDE_SANDBOXES=1
    ...
    ;;
  --configmap-name)
    CM_NAME="${2:?--configmap-name needs a name}"
    shift 2
    ;;
  --context)
    KUBECTL_CONTEXT="${2:?--context needs a name}"
    shift 2
    ;;
  --dry-run)
    DRY_RUN=1
